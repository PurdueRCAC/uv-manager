...
# Trampolines ignore UV_MANAGER_PLATFORM, so every tool breaks at a site that sets it

## Problem

The wrapper resolves its platform key as `${UV_MANAGER_PLATFORM:-$(uname -m)}` (`bin/uv-manager:145`),
but the generated trampoline hardcodes `a=$(uname -m)` (`bin/uv-manager:478`). The two disagree
whenever a site uses the override.

`UV_MANAGER_PLATFORM` is the documented escape hatch for exactly the cases where `uname -m` is too
...
`uv tool install ruff` lands the executable in `$UV_MANAGER_ROOT/<override-key>/bin/shims/ruff`, while
the trampoline on `PATH` looks for `$UV_MANAGER_ROOT/$(uname -m)/bin/shims/ruff`. That path does not
...
$ .agents/factory/bin/temp_root.sh --offline --keep --arch x86_64-glibc2.28 sh -c '
      uv --version >/dev/null
      mkdir -p "$UV_MANAGER_ROOT/x86_64-glibc2.28/bin/shims"
      printf "#!/bin/sh\necho hello\n" > "$UV_MANAGER_ROOT/x86_64-glibc2.28/bin/shims/ruff"
      chmod +x "$UV_MANAGER_ROOT/x86_64-glibc2.28/bin/shims/ruff"
      uvm trampolines
      "$UV_MANAGER_ROOT/bin/ruff"'
...
when a user's `UV_MANAGER_PLATFORM` differs from the one in effect when the trampolines were written.
...
A site that sets `UV_MANAGER_PLATFORM` gets trampolines that resolve to the same tree the wrapper
writes to, and the "not installed for this architecture" message names the key actually in use.

## Sketch of the acceptance criteria

- **R1** — WHILE `UV_MANAGER_PLATFORM` is set, WHEN a generated trampoline is invoked, it SHALL exec
...
- Decide whether a trampoline should honor `UV_MANAGER_PLATFORM` from the *invoking* environment or
