...
Part of the factory's self-improvement loop. The lifecycle skills append markdown
``## F<n>`` finding sections to ``spec/<slug>/META.md`` (silence by default);
``uvm-publish`` and ``/uvm-harness`` read them through this script rather than
parsing the markdown themselves. Mirrors ``next_phase.py``'s principle: the
*script* owns the fragile parsing, the model executes.

The finding format is deliberately **not** YAML, so this reader needs no
dependency and cannot corrupt the file on append: each finding is a
``## F<n> — title`` header, a single backtick ``key=val`` metadata line, and
``- **Key:** value`` bullets. A schema example inside a fenced code block (as in
the template) is skipped — only findings appended outside a fence are real.

Being stdlib-only, this runs under bare ``python3`` as well as ``uv run``.

Usage:
    uv run .agents/factory/bin/meta_status.py spec/<slug>/META.md
    uv run .agents/factory/bin/meta_status.py spec/<slug>/META.md --status open
    uv run .agents/factory/bin/meta_status.py spec/<slug>/META.md --severity high --id F1 F3

A missing file is a valid empty state (``exists: false``, no findings, exit 0): a
feature that logged no harness friction has no META.md.
...
def parse_findings(text: str) -> list[dict[str, object]]:
    ...
    # Flatten metadata and bullet fields into a stable record.
