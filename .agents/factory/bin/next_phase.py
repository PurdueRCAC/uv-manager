...
The ``uvm-build`` skill runs this at the top of every invocation instead of
...
Usage:
    uv run .agents/factory/bin/next_phase.py spec/<slug>/TECH.md
    uv run .agents/factory/bin/next_phase.py --all   # portfolio view over spec/*/TECH.md
                                                     # (run from the repo root)

Exit codes: 0 ok · 2 parse/validation error (message on stderr). With ``--all`` a
corrupt file becomes an ``error`` entry in the report rather than a failure — the
portfolio view must not die on one bad record.
"""
...
from _fsm import FSMError, compute_next, split_frontmatter, validate  # noqa: E402
