...
The ``uvm-build`` and ``uvm-review`` skills call this to advance state rather than
hand-editing YAML — surgical in-place YAML edits corrupt indentation. It parses the
frontmatter, applies the requested mutations, re-serializes canonically, and writes
the file back with the body preserved verbatim.
...
Usage examples:
    ...
    # set an in_progress phase and update the hill honesty signal
    uv run .agents/factory/bin/set_phase.py spec/<slug>/TECH.md \
    ...
    uv run .agents/factory/bin/set_phase.py spec/<slug>/TECH.md \
        --add-phase P7 --name "F1 remediation: honor the platform override" \
        --satisfies R4 --depends-on P6 --after P6 \
        --verify ".agents/factory/bin/lint.sh" \
        --current P7 --top-status in_progress --touch

    # retune a reopened phase's gate in place (remediation tightening a weak verify)
    uv run .agents/factory/bin/set_phase.py spec/<slug>/TECH.md \
        --phase P3 --verify ".agents/factory/bin/temp_root.sh --offline uvm status" --touch
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    ap.add_argument("--hill", choices=["uphill", "crest", "downhill"],
                    help="risk/honesty signal for --phase")
    ...
    ap.add_argument("--add-phase", metavar="ID",
                    help="add a new pending phase with this id (requires --name and --verify)")
    ap.add_argument("--name", help="phase name (for --add-phase, or to rename an existing --phase)")
    ap.add_argument("--satisfies",
                    help="comma-separated GOAL R-IDs (for --add-phase or an existing --phase; '' clears)")
    ap.add_argument("--depends-on", dest="depends_on",
                    help="comma-separated prerequisite phase ids (for --add-phase or an existing --phase; '' clears)")
    ap.add_argument("--after",
                    help="insert the new phase after this phase id (default: append last; --add-phase only)")
    ap.add_argument("--verify",
                    help="verify command (for --add-phase, or to retune an existing --phase's gate)")
    ...
    ap.add_argument("--verdict", choices=["none", "changes-requested", "approved"],
                    help="set review.verdict")
...
def main(argv: list[str]) -> int:
    ...
    if args.phase:
        ...
        # A reopened phase can retune its body in place — remediation tightening a stale gate.
        # name/verify may not be blanked; an empty --satisfies/--depends-on clears the list.
